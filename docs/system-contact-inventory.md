...
| _Added 2026-08-08:_ **the wall clock, for the lock surface's own time** (`LockClockView` + `LockClock.secondsUntilNextMinute`) | PROTECTED by sampling, and it owns no observer on purpose. The time can move underneath the app — NTP, a user edit, a timezone change, a resume from system sleep — and every wake recomputes the wait from the instant it woke rather than accumulating, so a jump costs one stale minute and corrects itself. That is why there is no `NSSystemClockDidChange` row here to maintain: a system contact inside a view is the anti-pattern, and the failure it would prevent is smaller than the contact. **Not verified over the shield**: whether the wake still fires on a raised space under App Nap is open, and `scripts/probes/lockscreen-clock-tick.swift` is the instrument, with a dispatch-timer control and a redraw counter so "never fired", "fired stale" and "fired but never repainted" are distinguishable |
