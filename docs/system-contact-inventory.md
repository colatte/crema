...
| _Added 2026-08-07, source changed 2026-08-08:_ **MusicBrainz + the Cover Art Archive** (`CoverArtArchiveLookup`, opt-in and born off) | PROTECTED by indifference — the only contact that is allowed to simply not answer: every failure (offline, a 404 for a release group nobody uploaded art for, a decode that did not fit, a body too small to be a cover) is the same silence, and the surface is already complete with the player's own bytes. 8 s timeout; one request per track identity, never per position tick. Two hosts now, and only the metadata one is rate-limited: MusicBrainz publishes 1 req/s per IP and REFUSES a request with no `User-Agent`, so a burst is spaced by `RequestPacer` and the header is not optional — its absence turns the feature off silently for everyone behind that address (docs/DECISIONS.md: the-cover-comes-from-the-archive-not-the-store) |
| _Added 2026-08-08:_ **the wall clock, for the lock surface's own time** (`LockClockView` + `LockClock.secondsUntilNextMinute`) | PROTECTED by sampling, and it owns no observer on purpose. The time can move underneath the app — NTP, a user edit, a timezone change, a resume from system sleep — and every wake recomputes the wait from the instant it woke rather than accumulating, so a jump costs one stale minute and corrects itself. That is why there is no `NSSystemClockDidChange` row here to maintain: a system contact inside a view is the anti-pattern, and the failure it would prevent is smaller than the contact. **Measured over the shield 2026-08-08** and the reading is the opposite of what its raw numbers suggest: `scripts/probes/lockscreen-clock-tick.swift` delivered 2 of 6 owed wakes in 369 s, and `pmset -g log` dates that to the MACHINE sleeping (idle sleep 09:53:58 for 282 s, wake 09:58:40, on a Mac set to sleep after 1 minute), not to the timer failing. `Task.sleep` waits on an absolute `ContinuousClock` deadline, which counts through suspension, so the deadline is already past on resume and the next wake carries the current minute — reproduced with SIGSTOP across five boundaries, the pending sleep returning 2 ms after resume with the right time. The dispatch-timer control delivered the same 2, which is what rules out a scheduler-specific fault. So no observer is owed and no App Nap exemption is: the surface can hold a minute as old as the machine's sleep and can never show one, because the screen is dark for exactly that interval |
