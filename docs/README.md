...
- `assets/` — the images published with the docs: the showcase screenshots the README embeds and the web export of the app icon, plus two captures kept for reference that no page links yet — the notch at rest (`notch-compact.png`) and the Settings window (`settings-about.png`). Naming them here is what keeps a future sweep from reading them as litter.
