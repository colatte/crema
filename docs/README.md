...
- [system-contact-inventory.md](https://github.com/colatte/crema/blob/main/docs/system-contact-inventory.md) — the ~105 places the app touches a resource that can die underneath it, in five classes. A checklist for future review; its statuses are dated and must be re-verified before they are trusted.
...
- `assets/` — the images published with the docs: the showcase screenshots the README embeds, the web export of the app icon, and the README's download badge (`download-macos.png`, generated by `design/badge/makedownloadbadge.swift` — regenerate there, never edit by hand), plus two captures kept for reference that no page links yet — the notch at rest (`notch-compact.png`) and the Settings window (`settings-about.png`). Naming them here is what keeps a future sweep from reading them as litter.
