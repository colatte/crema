...
- [appcast.xml](appcast.xml) — Sparkle update feed (GitHub Pages serves /docs from main, so it goes live once this lands there). Today an intentionally empty placeholder; `scripts/release.sh` regenerates it on signing-identity builds, and feed items are never authored by hand (each enclosure needs an EdDSA signature only the release tooling produces).
