...
- [ ] **Dois testes flakam raro sob a suíte paralela** — `ChildProcessDeadlineTests.aHungOperationIsAbandonedAtTheDeadlineAndKilled` e `ChainedNowPlayingSourceTests.firesActiveSourceEndedOnAPromotion` falharam 1× cada em runs completos (rodada qualidade4-e-calibragem) e passam isolados e em re-runs. Diagnóstico (selo v1.2): nenhum dos dois toca processo real — ambos rodam sobre clock e operação fakes; o vetor é o handshake `waitForSleep`, que sob carga desiste em silêncio ao estourar o teto de yields e deixa o `advance` seguinte sem sleeper casado (no-op). Paliativo: re-run do run completo (o veredito segue a linha `Test run with N tests`). Robustecimento aplicado (selo v1.2): as esperas dos dois foram convertidas ao idioma advance-dentro-do-poll (`eventuallyOffActor { advance; cond }`), que torna o advance idempotente — reincidência a partir daqui indica um vetor diferente.
