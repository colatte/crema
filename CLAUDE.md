...
| Displays externos           | **Metade implementada.** A entrada existe: `BetterDisplayOSDSource` consome a notificação de OSD do BetterDisplay (`pro.betterdisplay.BetterDisplay.osd`) e desenha o HUD de brilho do display **interno** — é o que faz os dois apps coexistirem sem disputar a tecla. O sentido inverso (pedir ao BetterDisplay que *aplique* um nível, via betterdisplaycli / URL scheme / App Intents) e o Lunar (socket `lunar listen`) seguem roadmap; sem eles, display externo não ganha HUD — ver ROADMAP.md                                            |
...
│   │   └── External/        # BetterDisplayOSDSource (borda DNC) + BetterDisplayOSDTranslation (JSON do BetterDisplay → SystemHUD); Lunar segue roadmap
