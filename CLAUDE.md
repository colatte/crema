...
- Fonte de eventos expõe `updates: AsyncStream<TipoDeDomínio>` + `isAvailable() async -> Bool`. A tradução (dict do MediaRemote; o JSON do BetterDisplay na fonte externa planejada) acontece **dentro da fonte**, na borda — nunca acima dela. A fonte de now playing recebe também os hints `noteSeek`/`noteSeekFailed` (default no-op em extensão): o comando de seek viaja por canal separado sem caminho de volta, então uma fonte que extrapola posição localmente re-ancora por eles (docs/DECISIONS.md: scrub-grace).
...
- Esperas de teste sobre o clock fake são **limitadas e MainActor-fair**: `waitForSleep` faz polling com `Task.yield` **no MainActor** (fairness FIFO com as tasks que parkam os sleeps) e desiste após um teto — um handshake que espera um park que nunca virá falha alto na asserção seguinte em vez de pendurar a suíte. Motivo: o await por continuation sem prazo deadlockou a suíte inteira de forma determinística (rodada round1-a1-a3), e um yield fora do MainActor queima o teto no executor global antes de as tasks esperadas rodarem.
...
- [ ] **Testes de espera bounded flakam sob suíte paralela em máquina saturada** — histórico: `ChildProcessDeadlineTests.aHungOperationIsAbandonedAtTheDeadlineAndKilled` e `ChainedNowPlayingSourceTests.firesActiveSourceEndedOnAPromotion` (rodada qualidade4-e-calibragem, 1× cada). O vetor do teste do deadline era de PRODUÇÃO e está morto: `raceAgainstDeadline` chamava `onDeadline()` (o kill, que desenrola a operação noutra thread) antes de `race.finish(timedOutValue)`, e o finish tardio da operação morta podia vencer o single-resume — fix da rodada re-endereçamento comita o valor do timeout ANTES do kill (determinístico). O vetor restante é de INFRA de teste: no arranque frio da suíte paralela com a máquina saturada (reproduzido sob orquestração pesada externa), as esperas bounded (`eventually`/`eventuallyOffActor`/`waitForSleep`) esgotam o orçamento antes de as tasks do fixture progredirem — orçamento uniformizado em 20k yields (dezenas de ms; bounded, falha alto), e as esperas usam o idioma advance-dentro-do-poll. Paliativo remanescente: re-run do run completo (o veredito segue a linha `Test run with N tests`).
